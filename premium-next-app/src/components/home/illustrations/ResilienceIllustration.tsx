...
export default function ResilienceIllustration() {
    ...
    // 3 isometric layers
    const layers = [
        { id: "layer1", color: "#6d28d9", label: "Client" },   // violet-700
        { id: "layer2", color: "#4f46e5", label: "API" },      // indigo-600
        { id: "layer3", color: "#1e1b4b", label: "Database" }, // indigo-950
    ];
    ...
    return (
        <div ref={ref} className="relative h-40 w-full overflow-hidden rounded-xl bg-black/40 border border-white/5 my-4 flex items-center justify-center perspective-[800px]">
            ...
            <motion.div
                className="absolute inset-0 z-0 bg-gradient-to-t from-violet-500/20 to-transparent"
            ...
            <div className="relative flex flex-col items-center justify-center gap-1 z-10" style={{ transform: "rotateX(55deg) rotateZ(-45deg)", transformStyle: "preserve-3d" }}>
                ...
                <motion.div
                    className="absolute w-2 h-40 bg-gradient-to-b from-violet-300 to-transparent blur-[2px]"
